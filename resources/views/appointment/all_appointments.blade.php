    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('exportTable', () => ({
                    ...
                    fetch('{{ url('/randevu/listesi/get') }}')
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                console.error('Randevu verileri alınamadı.');
                            }
                        })
                        .catch(error => {
                ...
                renderTable() {
                    const rows = this.appointmentList.map(appointmentList => {
                        const appointmentDetailUrl = "{{ url('/randevu/detay') }}";
                        const appointmentDetailUrlWithId = `${appointmentDetailUrl}/${appointmentList.id}`;

                        return [
                            appointmentList.id,
                            appointmentList.customer ? appointmentList.customer.name : 'Bilinmeyen Müşteri',
                            appointmentList.artist ? appointmentList.artist.artist_name : 'Bilinmeyen Sanatçı',
                            appointmentList.start,
                            appointmentList.service ? appointmentList.service.service_name : 'Belirsiz',
                            appointmentList.toplam_odeme,
                            `
                            <a href="${appointmentDetailUrlWithId}" class="btn btn-primary btn-sm">
                                <svg style="margin-right: 10px;" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['ID', 'Müşteri Adı', 'Sanatçı Adı', 'Randevu Başlangıç', 'Hizmet', 'Toplam Ödeme', 'İşlem'],
