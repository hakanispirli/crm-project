        ...
        <div class="panel mt-6">
            <div class="md:absolute md:top-5 ltr:md:left-5 rtl:md:right-5">
                <div class="mb-5 flex flex-wrap items-center">
                    ...
                    <div x-data="modal">
                        <button type="button" class="btn btn-danger" @click="toggle">Yönetici Hesabı Ekle</button>
                                    ...
                                    <div class="p-5">
                                        <form id="addAdmin">
                                            ...
                                            <div class="relative mb-4">
                                                ...
                                                <input type="text" name="name" placeholder="Yönetici Adı Soyadı" required
                                            ...
                                            <div class="relative mb-4">
                                                ...
                                                <input type="text" name="telefon" placeholder="0555 100 10 10" required
                                                    class="form-input ltr:pl-10 rtl:pr-10" />
                                            </div>
                                            ...
                                            <div class="relative mb-5">
                                                ...
                                                <input type="password" name="password" placeholder="Yönetici Şifre" required
                                                    class="form-input ltr:pl-10 rtl:pr-10" autocomplete="current-password" />
                                            </div>
                                            ...
                                            <div class="relative mb-5">
                                                ...
                                                <select name="roles" class="form-select text-white-dark ltr:pl-10 rtl:pr-10" required>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('exportTable', () => ({
                    ...
                    fetch('{{ url('/get/yoneticiler') }}')
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                console.error('Yönetici verileri alınamadı.');
                            }
                        })
                        .catch(error => {
                ...
                renderTable() {
                    const rows = this.user.map(user => {
                        const editUserUrl = "{{ url('/yonetici/duzenle') }}";
                        const editUserUrlWithId = `${editUserUrl}/${user.id}`;
                        const yoneticiId = user.id;
                        const adminImage = user.gorsel ? `<img src="{{ url('upload/admin_images/') }}/${user.gorsel}" class="h-12 w-12 rounded-md object-cover ltr:mr-3 rtl:ml-3">` : '<img src="{{ url('upload/profile.png') }}" class="h-12 w-12 rounded-md object-cover ltr:mr-3 rtl:ml-3">';
                        const yoneticiRole = user.roles.map(role => {
                            return `<span class="badge bg-secondary shadow-md dark:group-hover:bg-transparent">${role.name}</span>`;
                        }).join(' ');
                        return [
                            user.id,
                            adminImage,
                            ...
                            user.email,
                            user.telefon,
                            `<div>${yoneticiRole}</div>`,
                            `<div class="flex">
                            <a href="${editUserUrlWithId}" class="btn btn-sm btn-outline-primary mr-2">
                                Düzenle
                            </a>
                            <a onclick="deleteYonetici(${yoneticiId})" class="btn btn-sm btn-outline-danger">
                                Sil
                            </a>
                        </div>`
                        ];
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['ID', 'Profil Görseli', 'Yönetici Adı Soyadı', 'Yönetici E-Posta', 'Yönetici Telefon', 'Yönetici Rolü', 'İşlem'],
                            data: rows,
                        },
                        perPage: 10,
        ...
        function deleteYonetici(yoneticiId) {
            Swal.fire({
                title: 'Emin misiniz?',
                html: "<div style='margin-top: 20px;'>Yöneticiyi silmek istediğinizden emin misiniz?</div>",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('/yonetici/sil') }}/${yoneticiId}`,
                        type: 'POST',
                        headers: {
                        ...
                        success: function(response) {
                            Swal.fire(
                                'Başarılı!',
                                'Yönetici başarılı bir şekilde silindi.',
                                'success'
                            ).then(() => {
                                location.reload();
                            });
    ...
    <script>
        $(document).ready(function() {
            $('#addAdmin').submit(function(event) {
                ...
                var addYoneticiUrl = "{{ url('/yonetici/ekle') }}";
